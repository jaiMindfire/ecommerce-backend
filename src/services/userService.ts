...
export const loginUser = async (
  ...
  try {
    ...
    if (!user) throw new CustomError("Please sign up", 404); // Throw error if user not found

    // Check if the provided password matches the stored password
    const isMatch = await user.comparePassword(input.password);
    if (!isMatch) throw new CustomError("Invalid credentials", 401); // Throw error if passwords don't match
